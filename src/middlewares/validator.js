...
import { existenteTeacherEmail, existenteStudentEmail, esRoleValido } from "../helpers/db-validator.js";

export const registerTeacherValidator = [
    body("name", "The name is required!").not().isEmpty(),
    body("surname", "The surname is required!").not().isEmpty(),
    body("email", "You must enter a valid email!").isEmail(),
    body("email").custom(existenteTeacherEmail),
...
export const registerStudentValidator = [
    ...
    body("surname", "The surname is required!").not().isEmpty(),
    body("email", "You must enter a valid email!").isEmail(),
    body("email").custom(existenteStudentEmail),
    body('role').custom(esRoleValido),
    body("password", "Password must be at least 8 cahracters!").isLength({ min: 8 }),
    validarCampos
];
